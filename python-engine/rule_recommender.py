...
def _humanize_recommendation(role_title, n_postings, required, preferred):
    role_lower = role_title.lower()

    if required and preferred:
        req_skills = ', '.join(required[:-1]) + f' and {required[-1]}' if len(required) > 1 else required[0]
        pref_skills = ', '.join(preferred[:-1]) + f' and {preferred[-1]}' if len(preferred) > 1 else preferred[0]
        return (
            f"Based on an analysis of {n_postings} {role_lower} positions, "
            f"the core skills employers look for are {req_skills}. "
            f"Additional skills that can strengthen your profile include {pref_skills}."
        )
    elif required and not preferred:
        req_skills = ', '.join(required[:-1]) + f' and {required[-1]}' if len(required) > 1 else required[0]
        return (
            f"After reviewing {n_postings} {role_lower} job postings, "
            f"the essential skills required are {req_skills}."
        )
    elif preferred and not required:
        pref_skills = ', '.join(preferred[:-1]) + f' and {preferred[-1]}' if len(preferred) > 1 else preferred[0]
        return (
            f"We analyzed {n_postings} {role_lower} positions and found that "
            f"employers commonly value {pref_skills}. "
            f"These skills appear frequently across postings and are worth prioritizing."
        )
    else:
        return (
            f"We reviewed {n_postings} {role_lower} postings but could not identify "
            f"a consistent skill pattern. Consider researching specific job descriptions "
            f"in this field for more tailored guidance."
        )
    ...
    def _build_rules(self, jobs_df: pd.DataFrame) -> List[JobRole]:
        ...
        for category, group in jobs_df.groupby("role_category"):
            ...
            roles.append(JobRole(
                ...
                recommendation_text=_humanize_recommendation(title, n_postings, required, preferred),
            ))
        return roles
